/**
 * esbuild wrapper for task_recall outbox + conflict proof.
 */
...
const outfile = path.join(root, 'scripts/_prove-task-recall-outbox.bundle.cjs');

async function main() {
  await esbuild.build({
    entryPoints: [path.join(__dirname, 'prove-task-recall-outbox-e2e.ts')],
    bundle: true,
    platform: 'node',
    format: 'cjs',
    outfile,
