/**
 * esbuild wrapper for farmer_verification outbox + conflict proof.
 */
...
const outfile = path.join(root, 'scripts/_prove-farmer-verification-outbox.bundle.cjs');

async function main() {
  await esbuild.build({
    entryPoints: [path.join(__dirname, 'prove-farmer-verification-outbox-e2e.ts')],
    bundle: true,
    platform: 'node',
    format: 'cjs',
    outfile,
