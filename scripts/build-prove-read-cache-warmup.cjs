/**
 * esbuild wrapper for the proactive read-cache warm e2e proof.
 */
...
const outfile = path.join(root, 'scripts/_prove-read-cache-warmup.bundle.cjs');

async function main() {
  await esbuild.build({
    entryPoints: [path.join(__dirname, 'prove-read-cache-warmup-e2e.ts')],
    bundle: true,
    platform: 'node',
    format: 'cjs',
    outfile,
