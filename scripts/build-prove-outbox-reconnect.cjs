/**
 * esbuild wrapper for the outbox reconnect e2e proof.
 */
...
const outfile = path.join(root, 'scripts/_prove-outbox-reconnect.bundle.cjs');

async function main() {
  await esbuild.build({
    entryPoints: [path.join(__dirname, 'prove-outbox-reconnect-e2e.ts')],
    bundle: true,
    platform: 'node',
    format: 'cjs',
    outfile,
    ...
    plugins: [
      {
        ...
        setup(build) {
          build.onResolve({ filter: /offlineOutbox$/ }, (args) => {
            ...
            if (args.path.includes('outboxConnectivitySync')) return undefined;
