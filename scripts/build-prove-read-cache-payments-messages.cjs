/**
 * esbuild wrapper for payments + messages read-cache proof.
 */
...
const outfile = path.join(root, 'scripts/_prove-read-cache-payments-messages.bundle.cjs');

async function main() {
  await esbuild.build({
    entryPoints: [path.join(__dirname, 'prove-read-cache-payments-messages-e2e.ts')],
    bundle: true,
    platform: 'node',
    format: 'cjs',
    outfile,
