/**
 * esbuild wrapper for project_assign outbox + conflict proof.
 */
...
const outfile = path.join(root, 'scripts/_prove-project-assign-outbox.bundle.cjs');

async function main() {
  await esbuild.build({
    entryPoints: [path.join(__dirname, 'prove-project-assign-outbox-e2e.ts')],
    bundle: true,
    platform: 'node',
    format: 'cjs',
    outfile,
