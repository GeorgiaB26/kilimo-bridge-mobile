/**
 * esbuild wrapper for task_approval outbox + conflict proof.
 */
...
const outfile = path.join(root, 'scripts/_prove-task-approval-outbox.bundle.cjs');

async function main() {
  await esbuild.build({
    entryPoints: [path.join(__dirname, 'prove-task-approval-outbox-e2e.ts')],
    bundle: true,
    platform: 'node',
    format: 'cjs',
    outfile,
