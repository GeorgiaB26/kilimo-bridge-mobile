/**
 * esbuild wrapper for admin read-cache proof (dashboard / farmers / tasks).
 */
...
const outfile = path.join(root, 'scripts/_prove-read-cache-admin.bundle.cjs');

async function main() {
  await esbuild.build({
    entryPoints: [path.join(__dirname, 'prove-read-cache-admin-e2e.ts')],
    bundle: true,
    platform: 'node',
    format: 'cjs',
    outfile,
