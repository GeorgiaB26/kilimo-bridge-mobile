/**
 * esbuild wrapper for agent_task_approval outbox + conflict proof.
 */
...
const outfile = path.join(root, 'scripts/_prove-agent-task-approval-outbox.bundle.cjs');

async function main() {
  await esbuild.build({
    entryPoints: [path.join(__dirname, 'prove-agent-task-approval-outbox-e2e.ts')],
    bundle: true,
    platform: 'node',
    format: 'cjs',
    outfile,
