/**
 * esbuild wrapper for the offline task outbox e2e proof.
 * Redirects native offlineOutbox → web AsyncStorage implementation.
 */
const esbuild = require('esbuild');
const path = require('path');

const root = path.join(__dirname, '..');
const nativeOutbox = path.join(root, 'mobile/src/services/offlineOutbox.ts');
const webOutbox = path.join(root, 'mobile/src/services/offlineOutbox.web.ts');
const outfile = path.join(root, 'scripts/_prove-task-outbox-e2e.bundle.cjs');

async function main() {
  await esbuild.build({
    entryPoints: [path.join(__dirname, 'prove-task-outbox-e2e.ts')],
    bundle: true,
    platform: 'node',
    format: 'cjs',
    outfile,
  ...
  console.log('(native outbox redirected:', nativeOutbox, '→', webOutbox, ')');
}
