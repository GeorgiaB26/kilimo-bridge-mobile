...
export type PendingTaskApprovalView = {
  id: string;
  farmerTaskId: string;
  taskName: string;
...
function outboxItemToPendingApproval(item: OutboxItem): PendingTaskApprovalView {
  const farmerTaskId =
...
export async function listPendingTaskApprovals(): Promise<PendingTaskApprovalView[]> {
...
export async function dismissTaskApprovalOutbox(pendingId: string): Promise<void> {
  await deleteOutboxItem(pendingId);
}

/**
 * Enqueue-first task approve/reject with expected status pin.
 * Connectivity failures leave the row queued; conflicts stay as needs_review;
 * other failures remove the row and throw.
 */
export async function submitTaskDecisionWithOutbox(params: {
  farmerTaskId: string;
  taskName: string;
  decision: 'approve' | 'reject';
  /** Status the admin saw when deciding (usually submitted-for-approval). */
  expectedStatus: string;
  notes?: string;
  rejectionReason?: string;
}): Promise<
...
> {
  ...
  if (!expectedStatus) {
    throw new Error('expectedStatus is required for offline task decisions');
  }
  ...
  if (result.skipped) {
    throw new Error(result.error || 'Could not queue task decision');
  }
  ...
  throw new Error(result.error || extractApiError(undefined, 'Could not apply task decision'));
}

export async function pushPendingTaskApproval(
  pendingId: string
...
/** On admin tasks focus: try every queued task_approval that is still auto-syncable. */
export async function syncAllPendingTaskApprovals(): Promise<{
  synced: number;
  failed: number;
  needsReview: number;
}> {
