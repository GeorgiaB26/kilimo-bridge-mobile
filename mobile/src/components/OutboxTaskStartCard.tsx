...
import type { PendingTaskStartView } from '../services/submitTaskStartOutbox';

type Props = {
  item: PendingTaskStartView;
  pushing?: boolean;
  onPush?: () => void;
  onDismiss: () => void;
};

/** Queued farmer task start — distinguishes needs_review from pending/failed. */
export function OutboxTaskStartCard({ item, pushing, onPush, onDismiss }: Props) {
  const needsReview = item.status === 'needs_review';
  const sourceLabel =
    item.source === 'agent_assignment' ? 'Field agent assignment' : 'Program task';
    ...
    <KBCard elevated={false} style={{ marginBottom: 8 }}>
      <View className="flex-row items-start gap-2">
        ...
        <View className="flex-1">
          <Text className="text-base font-semibold text-[#333333]">{item.taskName}</Text>
          <Text className="mt-0.5 text-[13px] text-[#757575]">
            Queued start · {item.statusLabel}
          </Text>
          <Text className="mt-0.5 text-[12px] text-[#757575]">{sourceLabel}</Text>
          {item.startDate ? (
            <Text className="mt-0.5 text-[12px] text-[#757575]">Start date {item.startDate}</Text>
          ) : null}
          ...
          <>
            <Button className="h-10 bg-[#1A4D3E]" disabled={pushing} onPress={onPush}>
              {pushing ? (
                ...
                <Text className="text-white">Push start</Text>
              )}
            </Button>
