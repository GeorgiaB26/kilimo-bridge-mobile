...
import type { PendingTaskApprovalView } from '../services/submitTaskApprovalOutbox';

type Props = {
  item: PendingTaskApprovalView;
  pushing?: boolean;
  onPush?: () => void;
  onDismiss: () => void;
};
...
/**
 * Queued task approve/reject row — distinguishes needs_review from normal pending/failed.
 */
export function OutboxTaskApprovalCard({ item, pushing, onPush, onDismiss }: Props) {
  const needsReview = item.status === 'needs_review';
  const decisionLabel = item.decision === 'approve' ? 'Approve' : 'Reject';
    ...
    <KBCard elevated={false} style={{ marginBottom: 8 }}>
      <View className="flex-row items-start gap-2">
        ...
        <View className="flex-1">
          ...
          {item.syncError ? (
            <Text className={`mt-1 text-xs leading-[16px] ${needsReview ? 'text-[#D32F2F]' : 'text-[#757575]'}`}>
              {item.syncError}
            </Text>
          ) : null}
        </View>
