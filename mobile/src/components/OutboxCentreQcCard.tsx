...
import type { PendingCentreQcView } from '../services/submitCentreQcOutbox';

type Props = {
  item: PendingCentreQcView;
  pushing?: boolean;
  onPush?: () => void;
  onDismiss: () => void;
};

/** Queued centre QC — needs_review vs pending/failed. */
export function OutboxCentreQcCard({ item, pushing, onPush, onDismiss }: Props) {
  const needsReview = item.status === 'needs_review';
  const decisionLabel = item.decision === 'approve' ? 'Approve QC' : 'Reject QC';
    ...
    <KBCard elevated={false} style={{ marginBottom: 8 }}>
      <View className="flex-row items-start gap-2">
        ...
        <View className="flex-1">
          <Text className="text-base font-semibold text-[#333333]">{item.productName}</Text>
